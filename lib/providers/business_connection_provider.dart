...
class BusinessConnectionProvider extends ChangeNotifier {
  ...
  Future<void> fetchDiscoverProfiles() async {
    ...
    try {
      ...
      // Get all connections to exclude them from discover
      final myConnectionsResponse = await client
          .from('business_connections')
          .select('counterparty_id')
          .eq('user_id', userId);

      // Get list of connected user IDs to exclude
      List<String> connectedUserIds = [];
      if (myConnectionsResponse != null) {
        connectedUserIds = (myConnectionsResponse as List)
      ...
      // Fetch profiles that are not connected
