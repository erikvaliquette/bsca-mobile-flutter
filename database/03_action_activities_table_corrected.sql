...
-- Corrected version that references the user_actions table (not actions table)
...
CREATE TABLE IF NOT EXISTS action_activities (
    ...
    -- Optional organization reference (can be NULL if organizations table doesn't exist)
...
COMMENT ON COLUMN action_activities.action_id IS 'Reference to the parent strategic action in user_actions table';
