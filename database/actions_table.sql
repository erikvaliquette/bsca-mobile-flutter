...
-- This table supports the enhanced SDG structure: SDG → Target → Action
...
CREATE TABLE IF NOT EXISTS actions (
    ...
    -- SDG relationship
    sdg_target_id UUID REFERENCES sdg_targets(id) ON DELETE SET NULL,
    sdg_id INTEGER NOT NULL, -- Legacy field for backward compatibility
...
-- Users can view their own actions and actions from their organizations
...
COMMENT ON COLUMN actions.sdg_target_id IS 'Reference to specific SDG target this action addresses';
COMMENT ON COLUMN actions.sdg_id IS 'Legacy SDG ID field for backward compatibility';
